# Interiores de Quarteirão — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a Leaflet map showing surviving and lost private green spaces (block interiors) in Porto, detected from Sentinel-2 classifications with a building-neighbourhood filter and OSM park exclusion.

**Architecture:** Single Python script (`interiores_quarteiroes.py`) with 3 phases: (1) GEE classification + neighbourhood filter + PNG export, (2) OSM park mask applied locally with Pillow/Shapely, (3) HTML map generation with base64-embedded layers. Reuses classification logic from `porto_publish.py`.

**Tech Stack:** Google Earth Engine (Python API), requests, Pillow, Shapely, Leaflet 1.9.4

---

## File Structure

- **Create:** `interiores_quarteiroes.py` — single script (GEE + OSM mask + HTML generation)
- **Create:** `layers/interior_subsistente.png` — surviving block interiors (generated at runtime)
- **Create:** `layers/interior_perdido.png` — lost block interiors (generated at runtime)
- **Create:** `interiores_quarteiroes.html` — final interactive map (generated at runtime)

---

### Task 1: GEE setup and Sentinel-2 composites

**Files:**
- Create: `interiores_quarteiroes.py`

- [ ] **Step 1: Create script with GEE init, geometry, and composite functions**

Copy the GEE boilerplate and composite logic from `porto_publish.py`. This establishes authentication, the Porto bounding box, Sentinel-2 collection filtering, and multi-seasonal composite building.
...
- [ ] **Step 2: Run script to verify GEE connection works**

Run: `python interiores_quarteiroes.py`
Expected: prints "A calcular compositos Sentinel-2..." without errors.

- [ ] **Step 3: Commit**

```bash
git add interiores_quarteiroes.py
git commit -m "feat: interiores quarteiroes — GEE init and Sentinel-2 composites"
```

---

### Task 2: Classification and neighbourhood filter

**Files:**
- Modify: `interiores_quarteiroes.py`

- [ ] **Step 1: Add classification function and neighbourhood filter**

Append after the composite section. This classifies both epochs, computes the building-fraction neighbourhood, and derives the "lost" and "surviving" masks.
...
- [ ] **Step 2: Run to verify classification completes without errors**

Run: `python interiores_quarteiroes.py`
Expected: prints all 3 messages without errors. No PNGs generated yet.

- [ ] **Step 3: Commit**

```bash
git add interiores_quarteiroes.py
git commit -m "feat: interiores quarteiroes — classification and neighbourhood filter"
```

---

### Task 3: PNG download from GEE

**Files:**
- Modify: `interiores_quarteiroes.py`

- [ ] **Step 1: Add download function and layer exports**

Append after the classification section. Uses the same `download_layer` pattern from `porto_publish.py`.
...
- [ ] **Step 2: Run to download PNGs**

Run: `python interiores_quarteiroes.py`
Expected: downloads 3 PNGs to `layers/`. `interior_subsistente.png` and `interior_perdido.png` should be non-empty. `municipios.png` may be skipped if it already exists.

- [ ] **Step 3: Verify PNGs exist and have content**

Run: `python -c "from PIL import Image; i=Image.open('layers/interior_subsistente.png'); print(i.size, i.mode)"`
Expected: `(2048, ...)` `RGBA`
...
```bash
git add interiores_quarteiroes.py
git commit -m "feat: interiores quarteiroes — GEE PNG download"
```

---

### Task 4: OSM park mask

**Files:**
- Modify: `interiores_quarteiroes.py`

- [ ] **Step 1: Add OSM query and rasterization**

Insert this section after the PNG download, before the HTML generation. Queries Overpass API for park/garden polygons, rasterizes them to match the PNG grid, and applies the mask to both layers.
...
# Rasterize park mask to match PNG dimensions
# PNG bounds: lon [-8.70, -8.54], lat [41.13, 41.19]
def apply_osm_mask(filepath, parks_geom):
    ...
    # For each pixel, check if centroid falls inside a park polygon
    # Vectorized approach: create a grid of points and test containment
    from shapely.vectorized import contains
    xs = np.linspace(lon_min, lon_max, w)
    ys = np.linspace(lat_max, lat_min, h)  # top-to-bottom = north-to-south
    xx, yy = np.meshgrid(xs, ys)
    mask = contains(parks_geom, xx, yy)

    # Set alpha to 0 for park pixels
    arr[mask, 3] = 0
...
- [ ] **Step 2: Run to verify OSM mask is applied**

Run: `python interiores_quarteiroes.py`
Expected: prints park polygon count and number of masked pixels for each layer. PNGs are updated in place.

- [ ] **Step 3: Commit**

```bash
git add interiores_quarteiroes.py
git commit -m "feat: interiores quarteiroes — OSM park mask exclusion"
```

---

### Task 5: HTML map generation

**Files:**
- Modify: `interiores_quarteiroes.py`

- [ ] **Step 1: Add HTML generation with Leaflet map**

Append at the end of the script. Generates `interiores_quarteiroes.html` with 4 basemaps, 3 overlay layers, and a side panel. Follows the same pattern as `porto_publish.py` but with Positron as default and no color pickers.
...
Run: `python interiores_quarteiroes.py`
Expected: generates `interiores_quarteiroes.html`, opens in browser. Map shows Positron basemap with green (subsistente) and red (perdido) overlays. Basemap selector has 4 options.

- [ ] **Step 3: Visually verify the map**

Check:
- Positron basemap loads as default
- All 4 basemaps work in the dropdown
- Green spots appear in dense urban areas (block interiors with vegetation)
- Red spots appear where vegetation was replaced by buildings
- No large parks visible (Serralves, Cidade, Cristal should be masked out)
- Municipal boundaries visible
...
```bash
git add interiores_quarteiroes.py
git commit -m "feat: interiores quarteiroes — HTML map with Leaflet"
```
...
**Files:**
- Verify: `interiores_quarteiroes.py`, `interiores_quarteiroes.html`, `layers/interior_*.png`

- [ ] **Step 1: Delete cached PNGs and run from scratch**

Delete the generated PNGs to force a clean regeneration:

```bash
rm -f layers/interior_subsistente.png layers/interior_perdido.png
python interiores_quarteiroes.py
```

Expected: full pipeline runs, PNGs regenerated, HTML generated, map opens in browser.

- [ ] **Step 2: Verify no hardcoded credentials**

Run: `grep -n "GEE_PROJECT\|project=" interiores_quarteiroes.py`
Expected: only `os.environ["GEE_PROJECT"]` and `ee.Initialize(project=GEE_PROJECT)` — no hardcoded project IDs.

- [ ] **Step 3: Final commit**

```bash
git add interiores_quarteiroes.py layers/interior_subsistente.png layers/interior_perdido.png
git commit -m "feat: mapa de interiores de quarteirão completo"
```
