...
<div class="content">
    ...
    <div class="charts-grid">
        <div class="chart-card" style="grid-column: 1 / -1;">
            ...
            <canvas id="chartInternacional" style="min-height:340px"></canvas>
        </div>
    </div>
...
<script>
    ...
    // Comparação internacional — mortes de peões por 100 000 habitantes
    // Fonte: CDC MMWR 2022 (28 países de rendimento elevado) — apenas dados oficiais
    // Portugal: ANSR 2024, 1.03 peões/100k (nacional)
    var intlData = [
        {{ city: 'EUA', rate: 2.33, color: '#64748b' }},
        {{ city: 'Coreia do Sul', rate: 1.81, color: '#64748b' }},
        {{ city: 'Hungria', rate: 1.33, color: '#64748b' }},
        {{ city: 'Polónia', rate: 1.22, color: '#64748b' }},
        {{ city: 'Portugal', rate: 1.03, color: '#ef4444' }},
        {{ city: 'Japão', rate: 0.93, color: '#64748b' }},
        {{ city: 'França', rate: 0.74, color: '#64748b' }},
        {{ city: 'Reino Unido', rate: 0.59, color: '#64748b' }},
        {{ city: 'Alemanha', rate: 0.44, color: '#64748b' }},
        {{ city: 'Noruega', rate: 0.17, color: '#22c55e' }}
    ];
    ...
    new Chart(document.getElementById('chartInternacional'), {{
        ...
        data: {{
            ...
            datasets: [{{
                ...
                borderColor: intlData.map(d => d.city === 'Portugal' ? '#fff' : 'transparent'),
                borderWidth: intlData.map(d => d.city === 'Portugal' ? 2 : 0)
            }}]
        }},
        options: {{
            indexAxis: 'y',
            ...
            plugins: {{
                ...
                subtitle: {{ display: true, text: 'Fonte: CDC/MMWR, dados de 2022 — 28 países de rendimento elevado', color: '#64748b', font: {{ size: 11 }}, padding: {{ bottom: 10 }} }}
            }},
            scales: {{
                x: {{ ticks: {{ color: chartColors.text }}, grid: {{ color: chartColors.grid }}, title: {{ display: true, text: 'Mortes de peões por 100 000 hab./ano', color: chartColors.text }} }},
                y: {{ ticks: {{ color: function(context) {{ return intlData[context.index]?.city === 'Portugal' ? '#ef4444' : chartColors.text; }}, font: function(context) {{ return {{ weight: intlData[context.index]?.city === 'Portugal' ? 'bold' : 'normal', size: 13 }}; }} }}, grid: {{ color: chartColors.grid }} }}
            }}
        }}
    }});
